...
pub fn score_target(config: &AppConfig, metrics: &TargetMetrics) -> TargetScore {
    ...
    let packet_loss = metrics.icmp_packet_loss_rate.map(|loss| 1.0 - loss).unwrap_or(0.0);
    ...
    if is_single_period_sample(&metrics.test_period) {
        reasons.push("分时段样本不完整".into());
    }

    let performance_score = 100.0
        * (latency_score(metrics.avg_latency_ms, 300.0) * config.performance_weights.avg_latency
            + latency_score(metrics.p95_latency_ms, 600.0) * config.performance_weights.p95_latency
...
fn latency_score(value: Option<f64>, worst: f64) -> f64 {
    value.map(|value| 1.0 - (value / worst).min(1.0)).unwrap_or(0.0)
}

fn is_single_period_sample(test_period: &str) -> bool {
    matches!(test_period, "白天" | "晚上")
}

fn hops_score(value: Option<u32>) -> f64 {
    value.map(|value| 1.0 - (value as f64 / 30.0).min(1.0)).unwrap_or(0.0)
}
